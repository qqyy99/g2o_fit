...
public:
    ...
    void computeError() override {
        ...
        _error(0, 0) = _measurement - abcd(0, 0) * sin(abcd(1, 0) * t_ + abcd(2, 0)) - abcd(3, 0);
    }
    ...
    void linearizeOplus() override {
        ...
        const VertexParameters *v = static_cast<const VertexParameters *> (_vertices[0]);
        const Eigen::Vector4d abcd = v->estimate();
        _jacobianOplusXi[0] = - sin(abcd[1] * t_ + abcd[2]);
        _jacobianOplusXi[1] = - abcd[0] * cos(abcd[1] * t_ + abcd[2]) * t_;
        _jacobianOplusXi[2] = - abcd[0] * cos(abcd[1] * t_ + abcd[2]);
...
int main() {
    ...
    /***    开始（添加顶点）    ***/ 
    parameters->setEstimate(Eigen::Vector4d(1.0, 1.0, 0.0, 0.0));
    parameters->setId(0);
